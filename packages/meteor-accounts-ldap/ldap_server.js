...
LDAP.prototype.ldapCheck = function (options) {
    ...
    if (options.hasOwnProperty('username') && options.hasOwnProperty('ldapPass')) {
        ...
        // DN not provided, search for DN and use result to bind
            ...
            // Bind to LDAP using super user
            client.bind(self.options.superDn, self.options.superPass, function (err) {

                try {
                    // Error binding
                    if (err) {
                      console.error("Error binding to server with superDn. Server responded with:");
                    ...
                    client.search(self.options.base, searchOptions, function (err, res) {
                        console.log("Performing LDAP search to determine user DN with the following options:");
                        console.log("Base -> ", self.options.base);
                        ...
                        res.on('searchEntry', function (entry) {
                            ...
                            console.log("Found userDn: ", retObject.dn);
                            ...
                            client.bind(entry.objectName, options.ldapPass, function (err) {
                              try {
                                console.log("Performing bind with userDn");
                                if (err) {
                                  console.error("Error binding to server with userDn. Server responded with:");
                                  console.error(err.code, err.message);
                                  throw new Meteor.Error(err.code, err.message);
                                }
                                else {
                                  console.log("Bind succesful");
                                  ldapAsyncFut.return(retObject);
                                }
                              }
                              catch (e) {
...
Accounts.registerLoginHandler('ldap', function (loginRequest) {
    ...
    if (ldapResponse.error) {
        ...
        var user = Meteor.users.findOne({
            username: ldapResponse.searchResults.uid
        });
